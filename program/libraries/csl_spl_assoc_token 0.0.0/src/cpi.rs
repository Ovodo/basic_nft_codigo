...
pub fn create_idempotent(
	...
	msg!("CPI Instruction: CslSplAssocToken.CreateIdempotent");
	...
	invoke(
		&spl_associated_token_account::instruction::create_associated_token_account_idempotent(funding_info.key, wallet_info.key, mint_info.key, token_program_info.key),
		&[funding_info.clone(), assoc_token_account_info.clone(), wallet_info.clone(), mint_info.clone(), system_program_info.clone(), token_program_info.clone()],
	)
}